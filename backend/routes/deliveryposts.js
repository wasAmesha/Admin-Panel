...
router.route("/get/:id").get(async (req, res) => {
  ...
  await DeliveryPost.findById(deliveryPostID)
    .then((deliveryPost) => {
      res.status(200).send({ status: "post fetched", deliveryPost });
    })
    .catch((error) => {
      console.log(error);
...
//done by admin
//http://localhost:8070/deliverypost/count
router.get("/count", async (req, res) => {
  try {
    const count = await DeliveryPost.countDocuments();
    res.status(200).json({ count });
  } catch (error) {
    console.error("Error counting delivery posts:", error);
    res.status(500).json({ error: "Server error. Failed to count delivery posts." });
  }
});
