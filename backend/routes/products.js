...
//done by admin
//http://localhost:8070/product/count
router.get("/count", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Server error. Failed to count products." });
  }
});

//http://localhost:8070/product/categorycount
router.get("/categorycount", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Server error. Failed to count products by category." });
  }
});
