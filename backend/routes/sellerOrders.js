...
router.route("/get/:id").get(async (req, res) => {
  ...
  await SellerOrder.findById(sellerOrderID)
    .then((sellerOrder) => {
      res.status(200).send({ status: "post fetched", sellerOrder });
    })
    .catch((error) => {
      console.log(error);
...
//done by admin
//http://localhost:8070/sellerorder/count
router.get("/count", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Server error. Failed to count seller orders." });
  }
});
